...
exports.run = async (client, message, args, db, ayarlar, prefix, dil) => {
        ...
        if(!args[0]) {
          ...
          > • | **${prefix}sunucukur normal** --> Normal Sunucu Kurar.
          > • | **${prefix}sunucukur youtuber** --> Youtuber Sunucu Kurar.
          > • | **${prefix}sunucukur kod** --> Kod Sunucu Kurar.
          > • | **${prefix}sunucukur destek** --> Destek Sunucu Kurar..
          > • | **${prefix}sunucukur hosting** --> Hosting Sunucu Kurar.
          > • | **${prefix}sunucukur public** --> Public Sunucu Kurar.
          `)
          .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
          return message.channel.send(embed)
        }
        if(await db.fetch('kuruldu')) return message.channel.send(
        new Discord.MessageEmbed()
        .setDescription(`${global.deniedozel} **Şuanda başka bir sunucu kuruluyor lütfen bekleyin, bu biraz zaman alacaktır.**`)
        .setColor('RANDOM')
        ...
        if(args[0] === 'normal') {
           ...
           `).setImage('https://cdn.discordapp.com/attachments/489386777711214595/857904388109369344/sunucukur.gif')
           .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({ dynamic: true }))
           return message.channel.send(embed).then(normal => {
            message.channel.awaitMessages(response => response.content === 'evet', {
              ...
              message.author.send(new Discord.MessageEmbed().setColor('RANDOM').setDescription(`**:gear: Sunucunuzdaki kanalların, kategorilerin ve rollerin hepsinin silinip, botun sunucu kurmasını onayladınız! Sunucunuz kuruluyor, bu işlem biraz zaman alabilir.**`))
              .then(await db.set('kuruldu', true))
              setTimeout(async () => {
             ...
             message.guild.roles.create({
              data: {
                ...
                hoist: true,
                  ...
                  }).then(() =>message.author.send(new Discord.MessageEmbed().setColor('RANDOM').setDescription(`
                  **Sunucunuzun kurulumu tamamlandı!
