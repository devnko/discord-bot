...
exports.run = async (client, message, args, db, ayarlar, prefix) => {
  ...
  if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Yönetici yetkin yok!")
    ...
    if(!reason) {
        ...
        .setAuthor(`${client.ayarlar.botİsim} Bot`, message.author.avatarURL({dynamic: true}))
        .setColor(client.ayarlar.embedRenk)
        .setDescription(`Merhaba ${message.author}, Eğer bir kişiye uyarı vermek istiyorsan bir sebep belirtmen lazım!`)
        .setFooter(client.ayarlar.embedFooter, message.author.avatarURL({dynamic: true}))
        return message.channel.send(embed)
    }
    ...
    await db.push(`uyarı_${member.id}_${message.guild.id}`, { kullanıcı: member.id, sebep: reason, sunucu: message.guild.id, moderator: message.author.id, uyarısayı: id})
    
    const embed = new Discord.MessageEmbed()
    .setAuthor(`${client.ayarlar.botİsim} Bot`, message.author.avatarURL({dynamic: true}))
    .setColor(client.ayarlar.embedRenk)
    .setDescription(`Merhaba ${message.author}, Başarılı bir şekilde uyarı verdim!`)
