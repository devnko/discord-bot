...
const verilcekrol = await db.fetch(`${message.guild.id}.kayıtkızrol`)
if(!verilcekrol) return global.hata(message,`**Kaydolduğunda verilecek olan kız rolü ayarlamadan bu komut çalışamaz.**`,true)
...
if(!üye) return global.hata(message,`**Kayıt etmek için lütfen kız bir kullanıcı belirtin!**`,true)
...
await db.add(`${message.guild.id}.${message.author.id}.kayıtkızetti`, 1)
await db.add(`${message.guild.id}.${message.author.id}.toplamkayit`, 1)
await db.add(`${message.guild.id}.toplamkızkayit`, 1)
await db.add(`${message.guild.id}.toplamkayıt`, 1)
await db.push(`kayıterkeklog_${message.author.id}_${message.guild.id}`, { kayıtTürü: "Kız", kayıtEttiğiKişi: üye.id, toplamKayıtSayısı: await db.fetch(`${message.guild.id}.${message.author.id}.kayıterkeketti`) + await db.fetch(`${message.guild.id}.${message.author.id}.kayıtkızetti`), kayıtZaman: moment(Date.now()).format('DD/MM/YYYY HH:mm') })
...
.addField(":calendar_spiral: Discord'a Kaydolma Tarihi:", `${moment(üye.user.createdAt).format('DD')} ${abobayraknaptınnnnnnnnnn[moment(üye.user.createdAt).format('MM')]} ${moment(üye.user.createdAt).format('YYYY h:mm:ss')}`, true).then(
