...
let muteRole = await db.fetch(`muteRole_${msg.guild.id}`)
if(!muteRole) return;
  if (MessageData[msg.author.id].MesssageNumber === 6) {
      ...
      const sendeddd = new Discord.MessageEmbed()
        .setColor("BLUE")
      .setAuthor("FireOfEternity", client.user.avatarURL())
        .setDescription(`\`${msg.author.tag}\` Çok Hızlı Mesaj Atıyorsun Yavaşla Yoksa Susturulursun!`)
    .setFooter(`${client.ayarlar.footerEmbed}`, client.user.avatarURL())
      .setTimestamp()
  ...
  if (MessageData[msg.author.id].MesssageNumber === 15) {
    let id = makeid(10);
            ...
            await db.push(`uyarı_${msg.author.id}_${msg.guild.id}`, { kullanıcı: msg.author.id, sebep: "Spam Yapmak", sunucu: msg.guild.id, moderator: client.user.id, uyarısayı: id})

    if (msg.deletable) msg.delete();// yap koy bunu ok
      const spambed = new Discord.MessageEmbed()
        .setColor("RANDOM")
      .setAuthor("FireOfEternity", client.user.avatarURL())//sa as kanka çok az komut kaldı ha v12 yapılmamış .D 9 tane falan bide müzik sistemi eklicez o kadar
    ...
    await db.add(`spamuyarı10_${msg.guild.id}_${msg.author.id}`, 1)//değiştimi la silmesi vay amk
        await db.add(`günlük_spam`, +1)
    await db.add(`spamsayi_${msg.author.id}`, +1)
 ...
 return msg.channel.send(atıldı).then(atıldı => atıldı.delete({time:5000}))
    ...
    msg.member.ban({reason: `FireOfEternity Anti Spam Sistemi`})
      await db.delete(`spamuyarı10_${msg.guild.id}_${msg.author.id}`)//bittimi la
      const atıldı = new Discord.MessageEmbed()
      .setColor("BLUE")
      .setAuthor("FireOfEternity", client.user.avatarURL())
    ...
    msg.member.roles.cache.forEach(async a => {
      await db.set(`${msg.guild.id}.spam.${msg.author.id}.roles.${a.id}`, a.id )
      msg.member.roles.remove(a)//değiştimi la bunlar
    })
 msg.member.roles.add(muteRole)
    await db.set(`spamvar_${msg.guild.id}`, msg.author.id)
         var fetched = await msg.channel.messages.fetch({limit: MessageData[msg.author.id].MesssageNumber})
        var fetcheds = fetched.filter(m => m.author.id === msg.author.id).array().slice(0, MessageData[msg.author.id].MesssageNumber)
          msg.channel.bulkDelete(fetcheds)
    ...
    msg.channel.bulkDelete(10)
       ...
       msg.guild.members.cache.get(msg.author.id).roles.remove(muteRole)
              await db.delete(`spamvar_${msg.guild.id}`)
